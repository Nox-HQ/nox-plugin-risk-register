...
func TestScanFindsSecurityDebt(t *testing.T) {
	client := testClient(t)
	resp := invokeScan(t, client, testdataDir(t))

	found := findByRule(resp.GetFindings(), "RISK-001")
	if len(found) == 0 {
		t.Fatal("expected at least one RISK-001 (security debt) finding")
	}
}

func TestScanFindsDeprecatedAPI(t *testing.T) {
	client := testClient(t)
	resp := invokeScan(t, client, testdataDir(t))

	found := findByRule(resp.GetFindings(), "RISK-002")
	if len(found) == 0 {
		t.Fatal("expected at least one RISK-002 (deprecated API) finding")
	}
}

func TestScanFindsSinglePointOfFailure(t *testing.T) {
	client := testClient(t)
	resp := invokeScan(t, client, testdataDir(t))

	found := findByRule(resp.GetFindings(), "RISK-003")
	if len(found) == 0 {
		t.Fatal("expected at least one RISK-003 (single point of failure) finding")
	}
}

func TestScanFindsMissingRecovery(t *testing.T) {
	client := testClient(t)
	resp := invokeScan(t, client, testdataDir(t))

	found := findByRule(resp.GetFindings(), "RISK-004")
	if len(found) == 0 {
		t.Fatal("expected at least one RISK-004 (missing recovery) finding")
	}
}
